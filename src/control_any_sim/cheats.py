...
def get_selectable_sims(_connection=None):
    ...
    for sim_info in tgt_client.selectable_sims:
        output(
            "sim {} {} is selectable".format(sim_info.first_name, sim_info.last_name)
        )
...
def log_sim_info(_connection=None):
    ...
    output(
        "commodity_tracker_lod: {}".format(sim_info.commodity_tracker.simulation_level)
    )
